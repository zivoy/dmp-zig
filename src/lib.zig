...
export fn freeDiff(d: Diff) callconv(.C) void {
    allocator.free(std.mem.span(d.text));
}
...
pub const DiffOperation = enum(c_int) {
    delete = @intFromEnum(diff.DiffOperation.delete),
    equal = @intFromEnum(diff.DiffOperation.equal),
    insert = @intFromEnum(diff.DiffOperation.insert),
};
...
export fn diffFromDelta(text: [*c]const u8, delta: [*c]const u8, out_diffs_len: *c_int) callconv(.C) [*c]Diff {
    out_diffs_len.* = -1;
    const diffs = diff.diffFromDelta(allocator, std.mem.span(text), std.mem.span(delta)) catch @panic("OOM");
...
export fn matchMain(dmp: DiffMatchPatch, text: [*c]const u8, pattern: [*c]const u8, loc: c_int) callconv(.C) c_int {
    ...
    const res = match.matchMain(MatchContainer, allocator, dmp.match_distance, dmp.match_threshold, std.mem.span(text), std.mem.span(pattern), @intCast(loc)) catch null orelse return -1;
    return @intCast(res);
}
...
// export
fn patchMake(out_patches_len: *c_int, mode: c_int, ...) callconv(.C) [*c]Patch {
    var ap = @cVaStart();
    defer @cVaEnd(&ap);
    out_patches_len.* = -1;
    ...
    const res: patch.PatchList = switch (mode) {
        1 => blk: {
            ...
            break :blk patch.patchMakeStringString(allocator, std.mem.span(text1), std.mem.span(text2));
        },
        2 => blk: {
            ...
            break :blk patch.patchMakeDiffs(allocator, diff_int.items);
        },
        3 => blk: {
            const text1: [*c]const u8 = @cVaArg(&ap, [*c]const u8);
            ...
            const diffs_len: usize = @intCast(@cVaArg(&ap, c_int));
            const diff_int = dmpDiffListFromExtern(diffs[0..diffs_len]) catch @panic("OOM");
            break :blk patch.patchMakeStringDiffs(allocator, std.mem.span(text1), diff_int.items);
        },
        4 => blk: {
            const text1: [*c]const u8 = @cVaArg(&ap, [*c]const u8);
            const text2: [*c]const u8 = @cVaArg(&ap, [*c]const u8);
            ...
            const diffs_len: usize = @intCast(@cVaArg(&ap, c_int));
            const diff_int = dmpDiffListFromExtern(diffs[0..diffs_len]) catch @panic("OOM");
            break :blk patch.patchMakeStringStringDiffs(allocator, std.mem.span(text1), std.mem.span(text2), diff_int.items);
        },
...
export fn patchDeepCopy(patches: [*c]const Patch, patches_len: c_int) callconv(.C) [*c]Patch {
    ...
    const p_copy = patch.patchDeepCopy(allocator, i_patches) catch @panic("OOM");
...
fn patchApply(dmp: DiffMatchPatch, patches: [*c]const Patch, patches_len: c_int, text: [*c]const u8, out_applied: *[*c]bool) callconv(.C) [*c]const u8 {
    ...
    const result, const applied = patch.patchApply(MatchContainer, allocator, dmp.patch_delete_threshold, i_patches, std.mem.span(text)) catch @panic("ERR");
...
export fn patchAddPadding(dmp: DiffMatchPatch, patches: [*c]const Patch, patches_len: c_int, out_patches: *[*c]Patch, out_patches_len: *c_int) callconv(.C) [*c]const u8 {
    ...
    const padding = patch.patchAddPadding(allocator, dmp.patch_margin, &i_patches) catch @panic("OOM");
...
export fn patchSplitMax(dmp: DiffMatchPatch, patches: [*c]const Patch, patches_len: c_int, out_patches: *[*c]Patch, out_patches_len: *c_int) callconv(.C) void {
    ...
    patch.patchSplitMax(MatchContainer, allocator, dmp.patch_margin, &i_patches) catch @panic("OOM");
...
export fn patchToText(patches: [*c]const Patch, patches_len: c_int) callconv(.C) [*c]const u8 {
    ...
    const text = patch.patchToText(allocator, i_patches) catch @panic("OOM");
    return text.ptr;
}
...
export fn patchFromText(text: [*c]const u8, out_patches: *[*c]Patch, out_patches_len: *c_int) callconv(.C) void {
    ...
    const i_patches = patch.patchFromText(allocator, std.mem.span(text)) catch @panic("OOM");
...
export fn patchObjToString(p: Patch) callconv(.C) [*c]const u8 {
    ...
    const i_patch = dmpPatchFromExtern(p) catch @panic("OOM");
